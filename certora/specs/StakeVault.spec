...
methods {
  ...
  function StakeManager.accounts(address) external returns(uint256, uint256, uint256, uint256, uint256, address) envfree;
}

function getAccountBalance(address addr) returns uint256 {
  uint256 balance;
  _, balance, _, _, _, _ = stakeManager.accounts(addr);
